...
from text_metrics.metrics import aic
from text_metrics.resource_pool import rp as default_rp
from text_metrics.tools import spacy_tagger
import re
...
class SimpleWords(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        return aic.SimpleWords.value_for_text(self, t)
...
class CoordinateClausesPerClauses(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        clauses = spacy_tagger.num_clauses(t)
        coordinate_conjunctions = len(spacy_tagger.word_by_attribute(t, "pos_", "CCONJ"))
        try:
...
class SentencesWithZeroClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_without_clause = [ s for s in verbs_by_sentence if len(s) == 0 ]
        try:
            return len(verbs_without_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class SentencesWithOneClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        occurrences = 0
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_with_clause = [ s for s in verbs_by_sentence if len(s) == 1 ]
        try:
            return len(verbs_with_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class SentencesWithTwoClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        occurrences = 0
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_with_clause = [ s for s in verbs_by_sentence if len(s) == 2 ]
        try:
            return len(verbs_with_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class SentencesWithThreeClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        occurrences = 0
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_with_clause = [ s for s in verbs_by_sentence if len(s) == 3 ]
        try:
            return len(verbs_with_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class SentencesWithFourClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        occurrences = 0
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_with_clause = [ s for s in verbs_by_sentence if len(s) == 4 ]
        try:
            return len(verbs_with_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class SentencesWithFiveClause(base.Metric):
    """
        ...
        **Teste**: Estamos cadastrando ferros-velhos, mas não tivemos um trabalho forte contra a clonagem e agora vamos
        atacar para valer – admitiu Bacci.
    ...
    def value_for_text(self, t, rp=default_rp):
        occurrences = 0
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_with_clause = [ s for s in verbs_by_sentence if len(s) == 5 ]
        try:
            return len(verbs_with_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class SentencesWithSixClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        occurrences = 0
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_with_clause = [ s for s in verbs_by_sentence if len(s) == 6 ]
        try:
            return len(verbs_with_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class SentencesWithSevenMoreClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        occurrences = 0
        verbs_by_sentence = spacy_tagger.verbs_by_sentence(t)
        verbs_with_clause = [ s for s in verbs_by_sentence if len(s) >= 7 ]
        try:
            return len(verbs_with_clause) / len(verbs_by_sentence)
        except ZeroDivisionError:
            return 0
...
class ClausesPerSentece(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        sentences = len(list(spacy_tagger.sents(t)))
        verbos = len(spacy_tagger.verbs(t))
        try:
            return sentences / verbos
        except ZeroDivisionError:
            return 0
...
class CoordinateConjunctions(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        try:
            conj_s = len(spacy_tagger.word_by_attribute(t, "pos_", "SCONJ"))
            conj_c = len(spacy_tagger.word_by_attribute(t, "pos_", "CCONJ"))
            conj = len(spacy_tagger.word_by_attribute(t, "pos_", "CONJ"))
            conjuncoes = conj_s + conj_c + conj
            return conj_c / conjuncoes
        except ZeroDivisionError:
            return 0
...
class GerundVerbs(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        ger = spacy_tagger.word_by_feat_value(t,"VerbForm","Ger")
        verb = spacy_tagger.word_by_attribute(t, "pos_", "VERB")
        aux = spacy_tagger.word_by_attribute(t, "pos_", "AUX")
        try:
            return len(ger) / sum(
                [
                    len(verb),
...
class ParticipleVerbs(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        part = spacy_tagger.word_by_feat_value(t, "VerbForm","Part")
        verb = spacy_tagger.word_by_attribute(t, "pos_", "VERB")
        aux = spacy_tagger.word_by_attribute(t, "pos_", "AUX")
        try:
            return len(part) / sum(
...
class InfinitiveVerbs(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        inf = spacy_tagger.word_by_feat_value(t,"VerbForm","Inf")
        verb = spacy_tagger.word_by_attribute(t, "pos_", "VERB")
        aux = spacy_tagger.word_by_attribute(t, "pos_", "AUX")
        try:
            return len(inf) / sum(
...
class InflectedVerbs(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        fin = spacy_tagger.word_by_feat_value(t, "VerbForm", "Fin")
        verb = spacy_tagger.word_by_attribute(t, "pos_", "VERB")
        aux = spacy_tagger.word_by_attribute(t, "pos_", "AUX")
        try:
            return len(fin) / sum(
...
class PrepositionsPerSentence(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        ...
        preps = spacy_tagger.word_by_attribute(t,"pos_","ADP")
        prep_merge_cases = [ w for w in text if w.text.lower() in ("nela", "nele", "nelas", "neles", "dela", "dele", "delas", "deles") ]
        npreps = len(preps + prep_merge_cases)
        
        try:
            return npreps / len(sentences)
        except ZeroDivisionError:
            return 0
...
class RelativeClauses(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        try:
            verbos = len(spacy_tagger.verbs(t))
            rel = len(spacy_tagger.word_by_attribute(t,"dep_","acl:relcl"))
            return rel / verbos
        except ZeroDivisionError:
            return 0
...
class AppositionPerClause(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        try:
            verbos = len(spacy_tagger.verbs(t))
            appos = len(spacy_tagger.word_by_attribute(t, "dep_", "appos"))
            return appos / verbos
        except ZeroDivisionError:
            return 0
...
class AdverbialAdjunctPerClause(base.Metric):
    ...
    name = 'Mean of Adverbial Adjuncts Per Clause'
    column_name = 'adjunct_per_clause'

    def value_for_text(self, t, rp=default_rp):
        try:
            verbs = len(spacy_tagger.verbs(t))
            # Because spacy only parses dependencies, there's no adjunct notation.
            # So instead of adverbials, for spacy this metric considers the number of
            # adpositions, which by themselves would add text complexity (just see the
            # first sentence in the test text above, which is a chain of name adjuncts)
            count_adpos = 0
            for sentence in list(spacy_tagger.sents(t)):
                sentence_adpos = [w for w in sentence if w.__getattribute__("pos_") == "ADP"]
                count_adpos += len(sentence_adpos)
            return count_adpos / verbs
        except ZeroDivisionError:
            return 0
...
class FirstPersonPronouns(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        return aic.FirstPersonPronouns.value_for_text(self, t)
...
class SecondPersonPronouns(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        return aic.SecondPersonPronouns.value_for_text(self, t)
...
class ThirdPersonPronouns(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        return aic.ThirdPersonPronouns.value_for_text(self, t)
...
class FirstPersonPossessivePronouns(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        text = spacy_tagger.text(t)
        pronouns = [ w.text.lower() for w in text if w.text.lower() in rp.possessive_pronouns() ]
        person_pronouns = [ w for w in pronouns if w in rp.possessive_pronouns(1) ]
        try:
            return len(person_pronouns) / len(pronouns)
        except ZeroDivisionError:
            return 0
...
class SecondPersonPossessivePronouns(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        text = spacy_tagger.text(t)
        pronouns = [ w.text.lower() for w in text if w.text.lower() in rp.possessive_pronouns() ]
        person_pronouns = [ w for w in pronouns if w in rp.possessive_pronouns(2) ]
        try:
            return len(person_pronouns) / len(pronouns)
        except ZeroDivisionError:
            return 0
...
class ThirdPersonPossessivePronouns(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        text = spacy_tagger.text(t)
        pronouns = [ w.text.lower() for w in text if w.text.lower() in rp.possessive_pronouns() ]
        person_pronouns = [ w for w in pronouns if w in rp.possessive_pronouns(3) ]
        try:
            return len(person_pronouns) / len(pronouns)
        except ZeroDivisionError:
            return 0
...
class AuxiliaryParticipleSentences(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        text = spacy_tagger.text(t)
        aux_list = [w for w in text if (w.pos_ == "AUX" and text[w.i-1].pos_ != "AUX")] #To filter out some compound verbs (e.g. haver sido), which spacy tags as AUX AUX
        sentences = len(list(text.sents))
        occurrences = 0
        for aux in aux_list:
            if (aux.head.morph.to_dict().get("VerbForm") == "Part"):
                occurrences += 1
        try:
            return occurrences / sentences
        except ZeroDivisionError:
            return 0
...
class PassiveClauses(base.Metric):
    ...
    def value_for_text(self, t, rp=default_rp):
        passive = len(spacy_tagger.word_by_attribute(t,"dep_","aux:pass"))
        try:
            return passive / spacy_tagger.num_clauses(t)
        except ZeroDivisionError:
            return 0
...
class SpacyAIC(base.Category):
    name = 'AIC'
    table_name = 'aic'

    def __init__(self):
        super(SpacyAIC, self).__init__()
